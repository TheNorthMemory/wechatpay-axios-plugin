...
class Multipart extends Readable {
  ...
  append(field, value, filename = '') {
    ...
    data.push(Buffer.from(`Content-Disposition: form-data; name="${field}"${filename && isBufferOrStream ? `; filename="${filename}"` : ''}`));
    data.push(CRLF);
    ...
    data.push(CRLF);
    indices[field] = data.push(isBufferOrStream ? value : Buffer.from(String(value))) - 1;
    data.push(CRLF);
    data.push(dashDash);
    data.push(boundary);
    data.push(dashDash);
  ...
  entries() {} /* eslint-disable-line class-methods-use-this */

  set() {} /* eslint-disable-line class-methods-use-this */

  get(field) { return this.data[this.indices[field]]; }

  getAll() {} /* eslint-disable-line class-methods-use-this */

  has(field) { return !!this.indices[field]; }

  delete() {} /* eslint-disable-line class-methods-use-this */

  keys() { return Object.keys[this.indices]; }

  values() {} /* eslint-disable-line class-methods-use-this */
