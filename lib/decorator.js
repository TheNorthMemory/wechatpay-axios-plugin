...
class Decorator {
  ...
  static requestInterceptor() {
    return function signer(config) {
      ...
      // The server side guns away with 500 status code. Let's force using the single caculated value anyway.
  ...
  static responseVerifier(certs = {}) {
    return function verifier(data, headers, status) {
      ...
      const {
        [HTTP_WECHATPAY_TIMESTAMP]: timestamp, [HTTP_WECHATPAY_NONCE]: nonce, [HTTP_WECHATPAY_SERIAL]: serial, [HTTP_WECHATPAY_SIGNATURE]: signature,
      } = headers;
  ...
  request(pathname, method, data, config) {
    ...
    return this[flag ? V2 : V3].request(utils.merge(config, { data, url: url.slice(flag ? 3 : 0), method }));
  }
