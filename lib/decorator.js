...
class Decorator {
   ...
   *
   * @returns {function} - Named as given `path` function
   */
  request(pathname, method, data, config) {
    const detect = Object.prototype.hasOwnProperty.call
    const url = pathname.replace(/\{([^\/]+)\}/g, (tmpl, named) => detect(config, named) ? config[named] : tmpl)

    return this[/^\/v2/.test(url) ? V2 : V3].request(Object.assign({}, config, {data, url: url.replace(/^\/v2/, ''), method}))
  }
