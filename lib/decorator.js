...
class Decorator {
  ...
  static responseVerifier(certs = {}) {
    return function verifier(payload, {
      ...
      assert.ok(
        Rsa.verify(Formatter.response(timestamp, nonce, payload), signature, certs[serial]),
        `Verify the response payload with: timestamp=${timestamp}, nonce=${nonce}, signature=${signature}, cert={${serial}: publicCert} failed.`,
      );
