...
/**
 * Joined the inputs by a `Line Feed`(LF) char.
 *
 * @param {(string|number)[]} pieces - The string(s) joined by line feed.
 *
 * @returns {string} - The joined string.
 */
function joinedByLineFeed(...pieces) {
...
class Formatter {
  ...
  static castCsvBill(buffer) {
    const data = Buffer.from(buffer).subarray(3/* BOM(EFBBBF) */).toString().split(/\r?\n/);
  ...
  static request(method, uri, timestamp, nonce, body = '') {
    return joinedByLineFeed(method, uri, timestamp, nonce, body);
  }
  ...
  static response(timestamp, nonce, body = '') {
    return joinedByLineFeed(timestamp, nonce, body);
  }

  static joinedByLineFeed = joinedByLineFeed
