...
class Transformer {
  ...
  get verifier() {
    ...
    return function verifier(data, headers, status) {
      ...
      if (pathname && pathname === SIGNLESS[0] && utils.isObject(data) && data.retcode !== 0) {
      ...
      } else if (pathname && pathname !== SIGNLESS[0] && utils.isObject(data)
      ...
      } else if (SIGNLESS.includes(pathname)) {
        return data;
      }
