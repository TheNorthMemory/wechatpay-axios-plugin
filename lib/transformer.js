...
class Transformer {
  ...
  get verifier() {
    ...
    return function verifier(data, headers, status) {
      ...
      if (pathname && SIGNLESS.includes(pathname)) {
        if (pathname === SIGNLESS[0] && data && data.retcode !== 0) {
      ...
      if (data.return_code !== SUCCESS || (Object.prototype.hasOwnProperty.call(data, 'result_code') && data.result_code !== SUCCESS)) {
        return utils.buildBusinessError(
          { data, headers, status },
          'Verify the response\'s data is failed because %s\'s value is %s.',
