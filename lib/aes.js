...
class Aes {
  ...
  /**
   * @property {string} BLOCK_SIZES - The `aes` block sizes
   */
  static get BLOCK_SIZES() { return 16 }
  ...
  static get pkcs7() {
    ...
    return {
      ...
      padding: thing => {
        ...
        const pad = BLOCK_SIZES - payload.length % BLOCK_SIZES
      ...
      unpadding: thing => {
        const payload = Buffer.from(thing)
        const bit = Buffer.alloc(1)
        payload.copy(bit, 0, payload.length - 1)
        const pad = bit.readUInt8()
...
class AesGcm extends Aes {
  ...
  static decrypt(iv, key, ciphertext, aad = '') {
    ...
    const tag = buf.slice(-this.BLOCK_SIZES)
    const payload = buf.slice(0, -this.BLOCK_SIZES)
