...
class AesGcm extends Aes {
  ...
  static encrypt(iv, key, plaintext, aad = '') {
    const cipher = crypto.createCipheriv(this.ALGO_AES_256_GCM, key, iv).setAAD(Buffer.from(aad));
  ...
  static decrypt(iv, key, ciphertext, aad = '') {
    ...
    const decipher = crypto.createDecipheriv(this.ALGO_AES_256_GCM, key, iv);
...
class AesEcb extends Aes {
  ...
  static encrypt(plaintext, key, iv = null) {
    ...
    const cipher = crypto.createCipheriv(this.ALGO_AES_256_ECB, key, iv).setAutoPadding(false);
  ...
  static decrypt(ciphertext, key, iv = null) {
    ...
    const decipher = crypto.createDecipheriv(this.ALGO_AES_256_ECB, key, iv).setAutoPadding(false);
